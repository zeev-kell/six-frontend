...
class Particle {
  ...
  // 绘制点，计算第三点的位置和颜色
  draw(_1, _2) {
    const _3 = this.getNextPoint(_1, _2)
  ...
  // 获取下一个点Y轴
  getNext(y) {
    const next = y + (random() * 2 - 1.1) * this.opts.wave
    // 防止生成 溢出 或者 负数 点
  ...
  getColor() {
    ...
    const R = (Math.cos(this.opts.increase) * 127 + 128) << 16
    const G = (Math.cos(this.opts.increase + U / 3) * 127 + 128) << 8
    const B = Math.cos(this.opts.increase + (U / 3) * 2) * 127 + 128
    return '#' + (R | G | B).toString(16)
  }

  getCircleCenter(_1, _2, _3) {
    // 计算三点的中心为多边形的中心
    return {
      x: (_1.x + _2.x + _3.x) / 3 + (random() * this.opts.amplitude - 0.25) * this.opts.wave,
      y: (_1.y + _2.y + _3.y) / 3 + (random() * this.opts.amplitude - 0.25) * this.opts.wave,
    }
  }
  ...
  drawPolygon(_1, _2, _3, color) {
    this.ctx.beginPath()
    // 中心
    const c = this.getCircleCenter(_1, _2, _3)
    const sideLength = this.drawHelper.getSideLength() // -- 边数
    const initialAngle = this.drawHelper.getInitialAngle() // -- 角度
    const radius = this.drawHelper.getRadius() / this.opts.amplitude // -- 半径
    const angle = U / sideLength
  ...
  drawCircle(_1, _2, _3, color) {
    ...
    // 中心
    const c = this.getCircleCenter(_1, _2, _3)
    c.r = (random() * random() * 50) / this.opts.amplitude
    this.ctx.arc(c.x, c.y, c.r, 0, U, false)
    this.ctx.closePath()
    this.ctx.font = `${c.r}px bold Helvetica`
    this.ctx.textBaseline = 'middle'
    this.ctx.textAlign = 'center'
    this.ctx.fillText(GENE[~~(4 * random())], c.x, c.y + c.r / 10)
    this.ctx.fillStyle = color
    this.ctx.fill()
  }
