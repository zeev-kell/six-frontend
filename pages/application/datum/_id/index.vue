<template>
  <div class="datum-container">
    <div class="el-row el-row--flex is-align-middle datum-info">
      ...
      <div class="el-col el-col-16 text-truncate mx-0">
        ...
        <div class="el-row el-row--flex datum-tip">
          <div class="el-col">
    ...
    <el-tabs v-model="activeTab" class="datum-el-tabs" :before-leave="onBeforeLeave">
      <el-tab-pane label="资源介绍" name="application-datum-id-index" />
      <el-tab-pane v-if="isWork" label="工作结构" name="application-datum-id-index-work" />
      <el-tab-pane v-if="isApp" label="工具结构" name="application-datum-id-index-structure" />
      <el-tab-pane label="使用教程" name="application-datum-id-index-course" />
      <el-tab-pane v-if="isApp" label="运行案例" name="application-datum-id-index-case" />
      <el-tab-pane v-if="isApp" label="历史版本" name="application-datum-id-index-versions" />
    </el-tabs>
...
@Component({
  ...
  async middleware({ store, params, app }) {
    ...
    // ID 不同，需要重新请求数据
    if (params.id !== datum.data_id) {
      const item = await app.$api.datum.get(params.id)
      store.commit('data/UPDATE_CURRENT_DATUM', item)
    }
  },
})
export default class DatumIdIndex extends Vue {
  ...
  get isApp() {
    return this.$store.getters['datum/isSoftware']
  }
  get isWork() {
    return this.$store.getters['datum/isOperation']
  }
