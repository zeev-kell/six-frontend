  ...
  <div class="tool-box">
    <div v-for="(group, index) of toolList" :key="index" class="d-inline-b">
      <div class="el-button-group">
        <el-button v-for="btn of group" :key="btn.title" :title="btn.title" :icon="btn.icon" :type="btn.type" size="mini" @click="onClick(btn)" />
      </div>
    </div>
  </div>
</template>
...
export default class ToolBox extends Vue {
  ...
  mounted(): void {
    ...
    this.toolList = this.tools.split('|').map((tool): graphTool[] => {
      return tool.split(',').map((t): graphTool => this.BUTTON_LIST[t])
    })
  }
